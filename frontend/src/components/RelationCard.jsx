...
/** Relation dropdown options — grouped so maternal vs paternal are obvious. */
const RELATION_SELECT_GROUPS = [
    {
        label: "Children & siblings",
        keys: ["son", "daughter", "brother", "sister"],
    },
    {
        label: "Parents & spouse",
        keys: ["father", "mother", "spouse"],
    },
    {
        label: "Mother's side (maternal — great-grandparents & grandparents)",
        keys: [
    ...
    {
        label: "Father's side (paternal — great-grandparents & grandparents)",
        keys: [
    ...
    {
        label: "Grandchildren & extended family",
        keys: [
            "grandDaughter",
            "grandSon",
            "greatGrandDaughter",
...
function relationOptionIsVisible(optionKey, relationsList, relationship) {
    const opt = optionKey.toUpperCase();
    const relName = (relationship?.name || "").toUpperCase();

    const hasPaternalGP = relationsList.some((r) =>
        ["PATERNALGRANDFATHER", "PATERNALGRANDMOTHER"].includes(
            (r?.name || "").toUpperCase()
        )
    );
    const hasMaternalGP = relationsList.some((r) =>
        ["MATERNALGRANDFATHER", "MATERNALGRANDMOTHER"].includes(
            (r?.name || "").toUpperCase()
        )
    );

    // Great-grandparents on each side only after that side has at least one grandparent
    if (
        ["PATERNALGREATGRANDFATHER", "PATERNALGREATGRANDMOTHER"].includes(opt)
    ) {
        if (!hasPaternalGP) {
            return relName === opt;
        }
    ...
    if (
        ["MATERNALGREATGRANDFATHER", "MATERNALGREATGRANDMOTHER"].includes(opt)
    ) {
        if (!hasMaternalGP) {
            return relName === opt;
        }
    ...
    const hasGrandchild = relationsList.some(
        (r) =>
            (r?.name || "").toUpperCase() === "GRANDSON" ||
            (r?.name || "").toUpperCase() === "GRANDDAUGHTER"
    );
    const hasSon =
        relationsList.filter(
            (relation) => (relation?.name || "").toUpperCase() === "SON"
        ).length >= 2;
    const hasDaughter = relationsList.some(
        (relation) => (relation?.name || "").toUpperCase() === "DAUGHTER"
    );

    if (["GREATGRANDSON", "GREATGRANDDAUGHTER"].includes(opt)) {
        if (!hasGrandchild) {
            return relName === opt;
        }
    }

    if (!hasSon && opt === "DAUGHTERINLAW") {
        return relName === opt;
    }
    if (!hasDaughter && opt === "SONINLAW") {
        return relName === opt;
    }
    if (
        !hasDaughter &&
        !hasSon &&
        (opt === "GRANDSON" || opt === "GRANDDAUGHTER")
    ) {
        return relName === opt;
    }
...
export const RelationCard = ({
    ...
    async function handleUpload(image) {
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/upload-relation-photo`,
                {
                    method: "POST",
                    body,
                    headers: {
    ...
    useEffect(() => {
        if (relationship?.dod) {
            setDateOfDeath(
                relationship?.dod
                    ? formatDateStrToDate(relationship?.dod)
                    : new Date()
            );
    ...
    return (
        ...
        >
            <div>
                ...
                <div
                    className={"grid grid-cols-1 w-full sm:grid-cols-11 gap-4"}
                >
                    <div className={"col-span-5"}>
                        <div className="w-full">
                            ...
                            <div
                                className={
                                    "w-full border rounded-lg border-white/20"
                                }
                            >
                                <select
                                ...
                                >
                                    {RELATION_SELECT_GROUPS.map((group) => {
                                        const visibleKeys = group.keys.filter(
                                            (key) =>
                                                relationOptionIsVisible(
                                                    key,
                                                    values.relations,
                                                    relationship
                                                )
                        ...
                        <div className={"col-span-4"}>
                            <div className="w-full">
                                ...
                                <select
                                    ...
                                    required={
                                        relationship?.relation_id ? true : false
                                    }
                                    className={`${
                                        theme
                                            ? "bg-[#333333] border-white/20 placeholder-gray-400 text-white"
            ...
            <div className="date-selector">
                ...
                <SelectDatepicker
                    ...
                    key={index}
                    selectedDate={
                        relationship.dob == null || relationship.dob === ""
                            ? null
                            : relationship.dob instanceof Date
                            ? relationship.dob
                            : formatDateStrToDate(relationship.dob) || null
                    }
                    onDateChange={(date) => {
                        if (date == null) {
                            const newRelations = [...values.relations];
                ...
                {(() => {
                    if (relationship.dob == null || relationship.dob === "") {
                        return null;
                    }
                    const dobValue =
                        relationship.dob instanceof Date
                            ? relationship.dob
                            : formatDateStrToDate(relationship.dob);
