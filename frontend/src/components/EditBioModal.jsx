...
const validationSchemaPics = Yup.object().shape({
...
const validationSchemaStep5 = Yup.object().shape({
  spouse_facebook: Yup.string().matches(regex.facebook, "Invalid Facebook URL"),
  spouse_instagram: Yup.string().matches(
    regex.instagram,
    "Invalid Instagram URL"
  ),
  spouse_twitter: Yup.string().matches(regex.twitter, "Invalid Twitter URL"),
});
const fileTypes = [
  "JPG",
  "JPEG",
...
export const EditBioModal = ({
  ...
  useEffect(() => {
    ...
    initialValues = {
      ...
      profileUrl: userData?.profile_picture || "",
      dob: userData?.dob ? formatDateStrToDate(userData?.dob) : null,
      spouseDob: spouse?.dob ? formatDateStrToDate(userData?.dob) : null,
        ...
        <Form>
          ...
          >
            <div className="relative p-4 w-full max-w-[500px] sm:max-w-2xl max-h-full">
              <div
                className={`${
                  userData?.dark_theme ? "bg-[#242526]" : "bg-white"
              ...
              >
                ...
                >
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="relative z-[1]">
                          <label
                            ...
                            htmlFor="dob"
                          >
                            Add {values.name || userData?.name || "loved one"}’s
                            Date of Birth
                          </label>
                          <Field
                            name={"dob"}
                          ...
                          <div>
                            <label className="flex items-center gap-1">
                              <input
                                ...
                                checked={isPresent}
                                onChange={() =>
                                  handlePresentToggle(setFieldValue)
                                }
                  ...
                  {step === 2 && (
                    <div>
                      ...
                      {/* Render the bio with paragraph formatting */}
                      <p className="mt-4 ">
                        {values?.name || userData?.name}'s obituary:
                      </p>
                      <div className="formatted-bio border ">
                        {values.bio.split("\n").map((paragraph, index) => (
                      ...
                      {values.spouseBio ? (
                        <>
                          <p className="mt-4 ">Spouse obituary:</p>
                          <div className="formatted-bio border">
                            {values.spouseBio.length > 0
                              ? values.spouseBio
                                  .split("\n")
                                  .map((paragraph, index) => (
                  ...
                  {step === 5 && (
                    <div className="space-y-4">
                      <div className="mb-6 sm:text-lg">
                        <p>
                          Copy and paste{" "}
                          {values.name || userData?.name || "your loved one"}’s
                  ...
                  {step === 7 && (
                    <div>
                      ...
                      >
                        Search for{" "}
                        {values.name || userData?.name || "your loved one"}’s
                        final resting place by location or name. For exact
                        resting place, drop the pin at the plot of{" "}
                        {values.name || userData?.name || "your loved one"}.
                        <br />
                        <br />
                ...
                {step === 1 && (
                  <div>
                    <p
                      className={`${
                        userData?.dark_theme ? " text-white/70 " : "text-black"
                      }text-center md:text-end md:pr-6`}
                    >
                      For bio and other info, click Next.
                    </p>
                  </div>
                )}
                ...
                >
                  ...
                  >
                    <GrLinkPrevious className="mt-[2px]" /> Previous
                  </span>

                  {step < 7 && (
                    <button
                      onClick={() =>
                        handleSubmitSteps(values, { setSubmitting })
                      }
                      type="button"
                  ...
                  {step < 7 ? (
                    ...
                    >
                      Next <GrLinkPrevious className="mt-[2px] rotate-180" />
                    </button>
                  ) : (
                    <button
                      onClick={step > 5 && submitForm}
                      type="button"
                      disabled={!isValid || isSubmitting || step < 6}
