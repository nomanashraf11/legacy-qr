  ...
  relations: Yup.array().of(
    Yup.object().shape({
      ...
      dod: Yup.date()
        .nullable()
        .when(["dob"], (dob, schema) => {
          return dob && dob !== "null" && dob && dob !== "null"
            ? schema.min(dob, "Date of death must be after date of birth")
            : schema;
        }),
    })
  ),
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      let newRelations = relations?.map((item) => ({
        ...item,
        dob: dateYYYYMMDDFormat(item.dob),
                ...
                <div className="flex items-center justify-center p-4 border-b relative border-white/5 rounded-t">
                  ...
                  <div className="absolute top-0 right-0 left-0 bottom-0 flex items-center justify-end p-4">
                    <button
                      ...
                      type="button"
                      className={` ${
                        userData?.dark_theme ? "bg-black" : "bg-[#c4c1c1]"
                ...
                >
                  <div className="space-y-4">
                    <p className="block mb-6 sm:text-lg">
                      Add {userData?.name || "your loved one"}’s family members
                      below (optional)
                    </p>
                    <div className="space-y-10 pb-4">
                      {values.relations?.length > 0 &&
                        values?.relations?.map((relationship, index) => (
                          <RelationCard
                            theme={userData?.dark_theme}
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (values.relations) {
                          ...
                          const dob = subDays(today, daysToSubtract);

                          const daysToAdd = 1;
                          const dod = addDays(today, daysToAdd);
