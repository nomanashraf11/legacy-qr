...
/** Total gallery caps (existing + new) */
const MAX_PHOTOS = 50;
const MAX_VIDEOS = 5;
/** Files allowed in one submit batch (client requirement) */
const BATCH_UPLOAD_MAX = 25;
...
export const UploadPhotosModal = ({ open, setOpen, data, refetch }) => {
    ...
    const imageValidationSchema = Yup.object().shape({
        image: Yup.array()
            ...
            .max(
                BATCH_UPLOAD_MAX,
                `You can upload maximum ${BATCH_UPLOAD_MAX} images or videos at a time`
            )
    ...
    const handleSubmit = async (values, { resetForm }) => {
        if (values.youtube) {
            ...
            const total = values.image.length;
            let success = [];
            let photosThisBatch = 0;
            let videosThisBatch = 0;

            setIsBatchUploading(true);
            setBatchUploadProgress({ done: 0, total });
            ...
            try {
                for (let index = 0; index < values.image.length; index++) {
                    ...
                    if (item.file.type.includes("image")) {
                        if (
                            existingPhotosCount + photosThisBatch >=
                            MAX_PHOTOS
                        ) {
                            toast.error(
                                "Maximum photo count reached for this profile"
                            );
                            success.push(false);
                            setBatchUploadProgress({ done: index + 1, total });
                            continue;
                        }
                        try {
                        ...
                        if (
                            existingVideosCount + videosThisBatch >=
                            MAX_VIDEOS
                        ) {
                            toast.error(
                                "Maximum video count reached for this profile"
                            );
                            success.push(false);
                            setBatchUploadProgress({ done: index + 1, total });
                            continue;
                ...
                } else if (success.some((v) => v === true)) {
                    toast.info(
                        "Some files were uploaded; others failed or hit limits."
                    );
                }
            } finally {
                                    ...
                                    <div className="md:pr-20">
                                        <div className="space-y-4">
                                            <p className="sm:text-base text-sm">
                                                To add photos and videos click
                                                below. You can add up to{" "}
                                                {BATCH_UPLOAD_MAX} files per
                                                batch (photos and/or videos).
                                                For videos over one minute or
                                                300mb, please consider uploading
                                                via YouTube.
                                            </p>
                                            <div>
                                                            ...
                                                            <FileUploader
                                                                ...
                                                                handleChange={(
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        toast.warn(
                                                                            `You can add at most ${BATCH_UPLOAD_MAX} files per batch. Remove some to add more.`
                                                                        );
                                                                        return;
                                                                    }
                                                                    const toAdd =
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        toast.info(
                                                                            `Only ${remaining} file(s) added (batch limit is ${BATCH_UPLOAD_MAX}).`
                                                                        );
                                                                    }
                                                                    const imageObj =
                                                                        toAdd.map(
                                            ...
                                            {isBatchUploading &&
                                                ...
                                                batchUploadProgress.total >
                                                    ...
                                                    <div className="w-full">
                                                        ...
                                                        >
                                                            <div
                                                                className="h-full rounded-full bg-emerald-500/90 transition-[width] duration-200 ease-out"
                                                                style={{
                                                                    width: `${Math.min(
                                                                        100,
                                                                        Math.round(
